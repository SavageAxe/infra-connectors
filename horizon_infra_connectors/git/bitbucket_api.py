...
class GitAPI:
        ...
        *,
        ...
        if use_http_token:
            ...
            if not username_or_email:
                raise ValueError("Bitbucket basic authentication requires username_or_email")
            headers = {
                "Authorization": "Basic "
                + base64.b64encode(f"{username_or_email}:{token}".encode()).decode(),
                "Accept": "application/json",
            }

        api_base = base_url.rstrip("/")
        if is_server:
            if not re.search(r"/rest/api/\d+(?:\.\d+)?$", api_base):
                api_base = f"{api_base}/rest/api/1.0"

        self.api = BaseAPI(api_base, headers=headers).client
    ...
    async def _server_branch_head(self, branch: str) -> Optional[str]:
        """Return the latest commit hash for a Bitbucket Server branch."""
    ...
    async def _commit_server(
        ...
        *,
        ...
        source_commit = await self._server_branch_head(branch)
        ...
        for path in deleted:
            endpoint = _server_files_endpoint(self.project_key, self.repo_slug, path)
            params = {"message": commit_message, "branch": branch}
            if source_commit:
                params["sourceCommitId"] = source_commit
            response = await self.api.delete(endpoint, params=params)
            _handle_response(_safe_json(response), response.status_code)
            source_commit = await self._server_branch_head(branch)
